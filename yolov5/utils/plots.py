...
def heatmap(image, data):
    ...
    # overlay = overlay.fill(0)
    alpha = 0.5  # 设置覆盖图片的透明度
    cv2.rectangle(overlay, (0, 0), (image.shape[1], image.shape[0]), (255, 0, 0), -1)  # Define blue as the background color
    image = cv2.addWeighted(overlay, alpha, hit_img, 1 - alpha, 0)  # Blue background image added to src image
    # image = cv2.addWeighted(overlay, alpha, image, 1 - alpha, 0)  # Heat map added to blue background image
    # image = cv2.addWeighted(hit_img, alpha, image, 1 - alpha, 0)  # 将热度图覆盖到原图
    # image = cv2.addWeighted(hit_img, alpha, overlay, 1 - alpha, 0)  # 将热度图覆盖到原图
    return image


def myheatmap(image, data, ratio):
    '''
    image: src image
    data: coordinates
    ratio； The ratio between heatmap and src image
    '''
    image = cv2.resize(image, (int(image.shape[1] * ratio), int(image.shape[0] * ratio)),
                       interpolation=cv2.INTER_CUBIC)
    background = Image.new("RGB", (image.shape[1], image.shape[0]), color=0)
    # 开始绘制热度图
    data = [(int(i[0]* ratio), int(i[1]* ratio)) for i in data]
    hm = HeatMap(data)
...
def draw_ROI(event, x, y, flags, param):
    """
    Left button: Eating area points
    Right button: Finish drawing
    Middle button: Delete poly
    """
